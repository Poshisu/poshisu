...
- **Home / Chat:** `/chat` now serves as the Home surface, hydrates recent persisted chat turns from Supabase on refresh, keeps a readable tappable sticky daily kcal/macro summary visible while scrolling on mobile with a daily-value details sheet, and `/api/chat` uses the AI-CHAT-01B health-coach runtime with OpenAI-first provider selection, optional Anthropic switching, deterministic nutrition baselines for estimates, retrieved profile/memory context, markdown memory effects, concise best-guess-first estimate cards, structured assumptions/portion display, contextual thinking states, deterministic confirmed-ledger daily totals/table answers, explicit target-date confirmation for late-night/prior-day logs, food-only log-first confirmation cards, optional clarification copy, current IST date/hour context for the health coach, duplicate confirmation guardrails, and correction-safe confirm-save/adjustment loops; PostHog instrumentation and ElevenLabs transcription remain pre-beta follow-ups.
...
- **Profile / what Nourish knows:** Implemented as an end-user notes surface with editable profile/pattern notes, recent-change history, privacy export, and guarded delete-account controls.
