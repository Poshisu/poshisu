...
export const completeOnboardingAction = withServerActionLogging("completeOnboarding", async (rawInput: unknown) => {
  ...
  const { error: profileMemoryError } = await supabase.from("memories").upsert(
    ({
      user_id: user.id,
      layer: "profile",
      key: "main",
  ...
  const { error: patternsMemoryError } = await supabase.from("memories").upsert(
    ({
      user_id: user.id,
      layer: "patterns",
      key: "main",
  ...
  const { error: userUpdateError } = await supabase
    .from("users")
    .update({ onboarded_at: new Date().toISOString(), estimation_preference: input.estimation_preference })
    .eq("id", user.id);
