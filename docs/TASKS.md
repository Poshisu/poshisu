...
| P1-001F | Add multimodal onboarding composer controls + attachment metadata scaffolding | `src/components/onboarding/ChatOnboardingFlow.tsx`, `src/components/onboarding/ChatOnboardingFlow.test.tsx` | Onboarding composer renders text/photo/camera/file/voice affordances and local message state supports `text \| image \| audio \| file` with attachment metadata placeholders | `pnpm run test -- src/components/onboarding/ChatOnboardingFlow.test.tsx` | done |
