...
| RDX-03 | Frontend Design System | Apply Nourish tokens globally and remove hardcoded black/white/thick borders in shared UI | `src/app/globals.css`, `src/components/ui/**`, `src/components/**` | Shared components use semantic tokens; tap targets >=44px; no visible debug placeholder copy in shared primitives | `pnpm run lint && pnpm run typecheck` | in_progress |
