...
| RDX-05 | Onboarding UX | Replace fake chat onboarding with progressive 8-step conversational flow + skip/recovery | `src/components/onboarding/**`, `src/app/(onboarding)/**`, `src/lib/onboarding/**` | One-question-per-screen flow with chips + optional input; partial saves recover after refresh; completion summary to Home | `pnpm run test:e2e -g onboarding && pnpm run test -- src/components/onboarding/ChatOnboardingFlow.test.tsx` | in_progress *(started: remove user-facing confidence badges and keep clarifier prompts human-readable/English-only)* |
