...
| AI-CHAT-01 | Real LLM-backed health coach runtime | Add Claude-backed provider path, deterministic fallback, retrieved profile/memory context, safe markdown memory effects, agent trace metadata, and prompt eval coverage | `src/lib/agents/health-coach/**`, `src/lib/agents/orchestrator.ts`, `src/app/api/chat/route.ts`, `prompts/agents/HEALTH_COACH.md`, `docs/ai-chat-01-architecture.md` | Chat uses Claude when configured, deterministic fallback when not, safety policy blocks high-risk requests before provider execution, context/memory lifecycle is documented and eval-covered | `pnpm run test -- src/lib/agents/orchestrator.test.ts src/lib/agents/health-coach/runtime.test.ts src/app/api/chat/route.test.ts src/lib/evals/prompt-evals.test.ts && pnpm run eval:prompts` | done |
