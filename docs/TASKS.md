...
|---|---|---|---|---|
| CHAT-FAST-LOG-001 | Reduce meal-logging over-questioning | Once a visible estimate exists, “please log/save/confirm this meal” carries pending estimate context and the health-coach prompt defaults to best-guess assumptions instead of more questions | `pnpm run eval:prompts && pnpm run test -- src/app/(app)/chat/ChatMealLogger.test.tsx src/lib/agents/orchestrator.test.ts` | done |
| CHAT-CARD-GROUNDING-001 | Keep meal prose, estimate card, and confirm payload aligned | LLM meal presentation is ignored when it does not overlap deterministic candidate items; bánh xèo logs produce a bánh xèo card, not stale/retrieved meal content | `pnpm run test -- src/lib/nutrition/pipeline.test.ts src/lib/agents/orchestrator.test.ts src/lib/agents/health-coach/runtime.test.ts src/lib/evals/prompt-evals.test.ts` | done |
...
- The composer includes a transcript region, accessible meal message input, Send button, submitting state, error/retry copy, and a confirm-save estimate card sourced from `/api/chat` response blocks.
- `/api/chat` now returns the orchestrator response blocks in its safe envelope so the client can render structured `meal_log_candidate` output.
- The orchestrator now attaches a `confirmPayload` to meal candidates, preserving deterministic nutrition estimate, parsed items, meal slot, confidence, safety flags, and source text for the existing confirm-save path.
- The auth/onboarding E2E journey now submits a real breakfast message through `/chat`, confirms the returned estimate, and checks that the saved breakfast appears on `/today`.
- Remaining UAT follow-ups after this remediation: chat quick-action chips (`S7-UAT-D05`) and beta implement-or-descope decisions for image/audio/file modalities (`S7-UAT-D02` through `S7-UAT-D04`).
...
| RDX-04 | Auth UX | Redesign welcome/sign-in/sign-up with premium mobile-first layout and updated copy | `src/app/page.tsx`, `src/app/(auth)/**`, `src/components/auth/**`, `docs/implementation/auth_redesign_notes.md` | Auth flows functionally unchanged; copy and visual hierarchy match brief; loading/error states included | `pnpm run test:e2e -g "auth|protected /chat redirects"` | done *(closure evidence: GitHub Actions run `26358607166` / job `77589938852` passed; Playwright proof artifact `playwright-e2e-proof-26358607166.zip` finalized, artifact ID `7184352005`)* |
| RDX-05 | Onboarding UX | Replace fake chat onboarding with progressive 8-step conversational flow + skip/recovery | `src/components/onboarding/**`, `src/app/(onboarding)/**`, `src/lib/onboarding/**` | One-question-per-screen flow with chips + optional input; partial saves recover after refresh; completion summary to Home | `pnpm run test:e2e -g onboarding && pnpm run test -- src/components/onboarding/ChatOnboardingFlow.test.tsx` | in_progress *(reopened 2026-05-24 per founder feedback that visual quality and hand-holding do not meet target state; prior closure evidence retained for functional behavior only)* |
| RDX-05A | Design System Foundation | Align global tokens, typography, and core form/button primitives with Nourish dark-luxury design baseline before screen rewrites | `src/app/globals.css`, `src/app/layout.tsx`, `src/components/ui/{button,input,textarea,select,card}.tsx`, `docs/TASKS.md`, `docs/DECISIONS.md` | Shared primitives expose approved palette/typography/radius/shadow semantics and 44px+ touch targets; no route-level behavior changes in this slice | `pnpm run lint && pnpm run typecheck` | in_progress |
| RDX-05B | Welcome/Auth Visual Parity | Rebuild landing + auth shell/forms to match target premium dark mobile-first language with accessible error containers and guided copy | `src/app/page.tsx`, `src/app/(auth)/**`, `docs/TASKS.md`, `docs/DECISIONS.md` | Hero/media split, premium copy hierarchy, segmented auth feel, and clear non-destructive error surfaces implemented without auth logic changes | `pnpm run lint && pnpm run typecheck` | in_progress |
| RDX-05C | Progressive Onboarding + Asset Infra | Replace chat-thread onboarding with true step-by-step progressive flow and add storage bucket baseline for app media assets | `src/components/onboarding/**`, `src/app/page.tsx`, `supabase/migrations/0012_storage_assets_bucket.sql`, `README.md`, `docs/TASKS.md`, `docs/DECISIONS.md` | Onboarding is one-screen-per-step with integrated final review/CTA in the same visual shell; landing hero supports desktop/mobile-specific image sources; storage buckets exist with user-scoped policies | `pnpm run lint && pnpm run typecheck && pnpm run test -- src/components/onboarding/ChatOnboardingFlow.test.tsx` | in_progress |
| RDX-05C.1 | Landing + Onboarding polish hardening | Remove remote-asset hero fragility by shipping local responsive hero assets and continue progressive onboarding shell polish | `public/images/*`, `src/app/page.tsx`, `src/components/onboarding/**`, `docs/TASKS.md`, `docs/DECISIONS.md` | Landing hero always renders (desktop/mobile variants) without remote CDN dependency; onboarding remains single-shell progressive flow with no detached confirmation card | `pnpm run lint && pnpm run typecheck && pnpm run test -- src/components/onboarding/ChatOnboardingFlow.test.tsx` | in_progress |
| RDX-05C.2 | Readability + Accessibility pass | Normalize contrast/readability and user-facing error copy on auth/onboarding; codify design system doc for UI agents | `docs/DESIGN_SYSTEM.md`, `src/app/(auth)/**`, `src/components/onboarding/**`, `docs/TASKS.md`, `docs/DECISIONS.md` | No low-contrast heading/body text on cream surfaces; validation errors are user-readable (no internal enum dumps); design system exists as canonical markdown for UI work | `pnpm run lint && pnpm run typecheck && pnpm run test -- src/components/onboarding/ChatOnboardingFlow.test.tsx` | in_progress |
| RDX-05C.3 | Visual fidelity + onboarding UX finalization | Resolve remaining screenshot defects: contrast, step flow readability, and final summary/CTA continuity | `src/components/onboarding/**`, `src/app/(auth)/**`, `docs/TASKS.md`, `docs/DECISIONS.md` | Step counter/title contrast is readable on all onboarding screens; diet input never leaks enum validation to users; final review is its own explicit step with clear CTA | `pnpm run lint && pnpm run typecheck && pnpm run test -- src/components/onboarding/ChatOnboardingFlow.test.tsx` | in_progress |
...
| AI-CHAT-01 | Real LLM-backed health coach runtime | Add provider-backed health-coach runtime, retrieved profile/memory context, safe markdown memory effects, agent trace metadata, and prompt eval coverage | `src/lib/agents/health-coach/**`, `src/lib/agents/orchestrator.ts`, `src/app/api/chat/route.ts`, `prompts/agents/HEALTH_COACH.md`, `docs/ai-chat-01-architecture.md` | Chat uses the selected LLM provider, safety policy blocks high-risk requests before provider execution, context/memory lifecycle is documented and eval-covered | `pnpm run test -- src/lib/agents/orchestrator.test.ts src/lib/agents/health-coach/runtime.test.ts src/app/api/chat/route.test.ts src/lib/evals/prompt-evals.test.ts && pnpm run eval:prompts` | done |
| CHAT-02 | Persistent chat and estimate UX cleanup | Hydrate `/chat` from persisted Supabase messages, restore the latest pending estimate after refresh, compact oversized estimate summary copy, and show item portion/structured assumption details | `src/app/(app)/chat/page.tsx`, `src/app/(app)/chat/ChatMealLogger.tsx`, `src/lib/agents/health-coach/**`, `prompts/agents/HEALTH_COACH.md` | Refreshing `/chat` preserves recent conversation; pending estimate card survives refresh until confirmed/dismissed; estimate summary is compact; items show concrete g/mL/household portions; assumptions are structured by portion/preparation/oil/confidence | `pnpm run test -- src/app/(app)/chat/ChatMealLogger.test.tsx src/lib/agents/health-coach/runtime.test.ts src/lib/openai/client.test.ts src/app/api/chat/route.test.ts src/lib/evals/prompt-evals.test.ts && pnpm run eval:prompts` | done |
| CHAT-03 | Chat interaction polish and revised estimate loop | Replace generic meal-loading copy with contextual thinking states, add a sticky compact daily nutrition summary during scroll, auto-scroll to the latest chat turn, persist confirmed macro/micro totals, and send pending estimate context when the user adjusts a confirmation card | `src/app/(app)/chat/ChatMealLogger.tsx`, `src/app/api/chat/route.ts`, `src/lib/agents/health-coach/**`, `src/lib/meals/confirm.ts`, `prompts/agents/HEALTH_COACH.md` | Regular recall/coaching messages no longer say “Estimating your meal”; daily kcal/macros remain visible while scrolling on mobile; newly confirmed meals update carbs/protein/fat/fibre totals; adjustment messages re-run the pending estimate and return a fresh confirmable card | `pnpm run test -- src/app/(app)/chat/ChatMealLogger.test.tsx src/lib/meals/confirm-save.integration.test.ts src/app/chat/confirm/route.test.ts src/app/api/chat/route.test.ts src/lib/agents/health-coach/runtime.test.ts src/lib/agents/orchestrator.test.ts src/lib/evals/prompt-evals.test.ts && pnpm run eval:prompts` | done |
| CHAT-04 | Canonical estimate regression hardening | Fix screenshot bug where LLM prose understood the meal but confirmation card fell back to unrelated `roti`; expand typed parser for the breakfast regression, ensure card/confirm payload share one structured candidate, add desktop Enter-to-send, and document lifecycle/non-goals for voice/photo/local-date follow-ups | `src/lib/nutrition/pipeline.ts`, `src/lib/agents/health-coach/deterministicFallback.ts`, `src/lib/agents/orchestrator.test.ts`, `src/app/(app)/chat/ChatMealLogger.tsx`, `docs/meal-estimate-lifecycle.md` | Regression breakfast parses whey/oats/skyr/seeds/fruit/kimchi and never outputs roti; card kcal/macros match confirm payload; Enter sends while Shift+Enter does not; lifecycle docs explain canonical estimate, local-date target, voice target, and photo target | `pnpm run test -- src/lib/nutrition/pipeline.test.ts src/lib/agents/orchestrator.test.ts src/app/(app)/chat/ChatMealLogger.test.tsx && pnpm run eval:prompts` | done |
| AI-CHAT-01B | OpenAI-first multi-provider health coach | Add OpenAI provider support, keep Anthropic switchability, remove deterministic/template chat fallback on provider failure, and document exact env setup | `src/lib/openai/client.ts`, `src/lib/agents/health-coach/**`, `src/app/api/chat/route.ts`, `src/app/api/health/llm/route.ts`, `.env.local.example`, `docs/ai-chat-01-architecture.md` | `NOURISH_LLM_PROVIDER=openai` uses `OPENAI_API_KEY` by default; `anthropic` remains selectable; missing/failed provider returns `503 LLM_UNAVAILABLE` instead of assistant template text; `/api/health/llm?check=1` provides authenticated provider diagnostics; matching `NAME=value` Vercel paste mistakes are normalized; tests/evals cover provider-required behavior | `pnpm run test -- src/lib/agents/orchestrator.test.ts src/lib/agents/health-coach/runtime.test.ts src/app/api/chat/route.test.ts src/lib/evals/prompt-evals.test.ts && pnpm run eval:prompts` | done |
...
| RDX-05D.1 | Canonical design system adoption | Replace sparse design-system doc with full strict spec and non-negotiable UI contracts. | `rg -n "Non-negotiable principles|Semantic token contract|CI enforcement policy" docs/DESIGN_SYSTEM.md` | done |
...
## 2026-06-05 — Founder QA follow-up: chat ledger and screenshot breakfast estimate

| ID | Lane | Task | Files likely touched | Acceptance criteria | Verify command | Status |
|---|---|---|---|---|---|---|
| QA-CHAT-LEDGER-01 | AI/Product Reliability | Connect daily nutrition table questions to confirmed meal ledger instead of free-form LLM memory | `src/lib/agents/health-coach/runtime.ts`, `src/lib/agents/health-coach/dailySummary.ts`, `src/lib/agents/health-coach/runtime.test.ts`, `docs/DECISIONS.md`, `README.md` | Daily/table/macros questions default to the requested local day, return confirmed meal rows plus day totals, avoid a meal card, and do not ask for the date when a date/today can be inferred | `pnpm run test -- src/lib/nutrition/pipeline.test.ts src/lib/agents/health-coach/runtime.test.ts` | done |
| QA-CHAT-LEDGER-02 | Nutrition Estimation | Add deterministic coverage for the screenshot breakfast items that were previously missing | `src/lib/nutrition/pipeline.ts`, `src/lib/nutrition/pipeline.test.ts` | Parser recognizes bún bò huế, bacon strips, smoked salmon, and hash brown with realistic default/count portions; estimates land in a plausible restaurant-breakfast range | `pnpm run test -- src/lib/nutrition/pipeline.test.ts src/lib/agents/health-coach/runtime.test.ts` | done |

## 2026-06-09 — Founder reset: log-first chat and end-user profile copy

| ID | Lane | Task | Files likely touched | Acceptance criteria | Verify command | Status |
|---|---|---|---|---|---|---|
| CHAT-UX-07 | Chat/AI/Product Reliability | Restore log-first confirmation behavior, reduce clarification loops, add time-aware context/date parsing, and harden duplicate confirmation guardrails | `src/lib/agents/health-coach/deterministicFallback.ts`, `src/lib/agents/health-coach/contextBuilder.ts`, `src/lib/meals/targetDate.ts`, `src/lib/meals/confirm.ts`, `prompts/agents/HEALTH_COACH.md`, `src/app/(app)/chat/ChatMealLogger.tsx` | Food-only meal messages create a confirmation card; optional clarifications do not block saving; current IST date/hour/time is available to the health coach prompt; natural dates like “12 June” are recognized; repeated confirmation of the same card is ignored across the target-day window | `pnpm run test -- src/lib/agents/orchestrator.test.ts 'src/app/(app)/chat/ChatMealLogger.test.tsx' src/lib/meals/confirm-save.integration.test.ts && pnpm run eval:prompts` | done |
| PROFILE-UX-02 | Frontend/Product | Replace builder-centric profile memory inspector copy with end-user Nourish notes language | `src/app/(app)/profile/ProfileMemoryDashboard.tsx`, `src/app/(app)/profile/ProfileMemoryDashboard.test.tsx` | Profile page explains what Nourish knows in user language; no “user-scoped”, “audit visible”, “memory inspector”, keys, or version badges appear as primary end-user concepts | `pnpm run test -- 'src/app/(app)/profile/ProfileMemoryDashboard.test.tsx'` | done |
