...
| RDX-05 | Onboarding UX | Replace fake chat onboarding with progressive 8-step conversational flow + skip/recovery | `src/components/onboarding/**`, `src/app/(onboarding)/**`, `src/lib/onboarding/**` | One-question-per-screen flow with chips + optional input; partial saves recover after refresh; completion summary to Home | `pnpm run test:e2e -g onboarding && pnpm run test -- src/components/onboarding/ChatOnboardingFlow.test.tsx` | done *(increment 1: removed user-facing confidence badges; increment 2: added local draft persistence + refresh recovery; increment 3: added explicit “Start over” recovery control; closure evidence: GitHub Actions run `26369531183` with App gates + DB scoped E2E passing and Playwright proof bundle `playwright-e2e-proof-26369531183.zip` attached)* |
